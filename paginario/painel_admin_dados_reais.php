...
function buscarEstatisticas($pdo) {
    $stats = [];
    ...
    try {
        ...
        // Total de editoras
        $stmt = $pdo->query("SELECT COUNT(*) as total FROM editora");
        $stats['editoras'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];
        
        // Total de gêneros
        $stmt = $pdo->query("SELECT COUNT(*) as total FROM genero");
        ...
        $stmt = $pdo->query("SELECT titulo, autor, DATE_FORMAT(NOW(), '%d/%m/%Y') as data_add FROM livro ORDER BY id_livro DESC LIMIT 5");
        $stats['ultimos_livros'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Últimas solicitações
        $stmt = $pdo->query("SELECT s.nome_livro, s.nome_autor, u.nome_completo as usuario FROM solicitacao s JOIN usuario u ON s.cpf = u.cpf ORDER BY s.id_solicitacao DESC LIMIT 5");
        $stats['ultimas_solicitacoes'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Livros por gênero
        $stmt = $pdo->query("SELECT g.nome_genero, COUNT(l.id_livro) as total FROM genero g LEFT JOIN livro l ON g.id_genero = l.genero_id GROUP BY g.id_genero, g.nome_genero ORDER BY total DESC LIMIT 5");
...
$estatisticas = buscarEstatisticas($pdo);
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
